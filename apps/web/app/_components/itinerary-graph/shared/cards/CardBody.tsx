...
// The shared glance body for a NodeResponse — the type-specific "signature
// detail" (flight boarding pass, hotel image + activities, experience photo +
// rating, meal, transit, note, free-time) that fills a glance-width CardShell.
//
// Extracted out of views/horizontal/NodeCard so EVERY card surface renders the
// same body: the timeline glance, the Collection card, and the chat proposal
// card all import `CardBody` from here (M006 harmonization). Living under
// shared/ keeps the import direction clean — collection/ and chat/ no longer
// reach into views/horizontal/.
...
import { Chip, Sub, Title } from "./CardShell";
import { TYPE_TOKENS, type CardKind, type StatusKind } from "./tokens";

// The node graph collapses every ground transport leg into the single `transit`
// type, but the card design has subway / train / drive / walk / boat. We infer
// from the mode/title — good enough for fixture data and lets each transit leg
// render with the right token color and icon.
...
function ImageStub({
  ...
  // Using next/image with remote loaders is overkill for fixtures; a plain <img>
  // degrades cleanly when /japan/* assets are missing. The gradient fallback
  // keeps the card readable in either case.
