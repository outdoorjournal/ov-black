...
// it is a view over the spine, never a parallel store. A card click routes to
// the full-bleed card detail (PS4); the DndContext is a harmless anchor so cards
// don't warn out of a provider and so PS5's pick-then-place has somewhere to
// hang. Scheduling from here (the cross-surface place-mode overlay) lands in PS5.
...
export function CollectionPlanningSpace() {
  ...
  return (
    ...
    <div className="flex min-h-0 flex-1">
      <DndContext>
        <CollectionRail variant="board" onOpenNode={openNode} />
      </DndContext>
    </div>
  );
}
