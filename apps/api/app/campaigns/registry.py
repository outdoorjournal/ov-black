...
OLYMPUS = Campaign(
    ...
    directive=(
        ...
        "``exact`` if they named firm dates. Ask ONCE. If they still won't give "
        "a length — evasive, undecided, or they'd rather you just handle it — do "
        "NOT keep pressing: the default is the full 14-night ascent (the whole "
        "mountain, unhurried), and it's easy to trim later. Either record 14 via "
        "``update_trip_timing`` (``window``, ``duration_nights=14``) or just move "
        "on to party — the dashboard lays down the 14-night spine by default "
        "when no length was settled. Never let a missing length stall intake or "
        "make you re-ask.\n"
