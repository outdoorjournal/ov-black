...
_FREE = (NodeStatus.idea, NodeStatus.proposed, NodeStatus.discarded)
...
def test_gate_allows_any_actor_on_pre_firmed(
    ...
    """idea / proposed / discarded are freely editable by everyone."""
    assert (
        _check_status_gate(
...
async def test_advisor_demotion_works_and_is_logged(db_session: AsyncSession) -> None:
    ...
    try:
        ...
        assert demoted.status is NodeStatus.proposed
        ...
        assert demotion.after["status"] == NodeStatus.proposed.value
    finally:
        await _cleanup(itinerary.id)
...
async def test_lock_reason_rides_on_graph_read(db_session: AsyncSession) -> None:
    ...
    try:
        ...
        assert by_id[proposed.id].lock_reason is None
    finally:
        await _cleanup(itinerary.id)
