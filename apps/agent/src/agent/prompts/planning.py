...
_RUBRIC_CLIENT = (
    ...
    "to add one. When the client has settled on a shape that covers the "
    "trip, call ``assemble_draft`` with a day-by-day ordering to create "
    "``follows`` edges between cards. Emit ``assemble_draft`` at most "
    "once per turn.\n\n"
...
_RUBRIC_ADVISOR = (
    ...
    "to reorder). Speak to the advisor as a peer — concise, technical "
