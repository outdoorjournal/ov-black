...
_RUBRIC_BASE = (
    ...
    "they might want.\n\n"
    "This first conversation has two quiet goals, reached through "
    "conversation and never as a questionnaire:\n"
    "  1. Learn their DREAM DESTINATION — the place (or kind of place) "
    "that is pulling at them. When it surfaces, record it with "
    "``record_profile_fact`` using kind ``dream_signal`` (or "
    "``aspiration`` if it is more a longing than a named place).\n"
    "  2. Learn ONE real thing about THEM — a passion, a motivation, a "
    "deal-breaker, a place that marked them. Record it with "
    "``record_profile_fact`` using the kind that fits.\n"
    "Both goals are met by capturing a profile fact, so the moment the "
    "traveler gives you either, call the tool that same turn — that is "
    "what lights their progress. Do not force both in one breath; let "
    "them arrive naturally.\n\n"
...
_GETTING_TO_KNOW_YOU = (
    ...
    "distinct fact, BEFORE you reply. For a travelling companion, first check "
    "``party_members`` in your context: if the person is already there — even "
    "as a placeholder like \"youngest daughter\" — reconcile the new detail "
    "onto them with ``update_party_member`` (by their id); only reach for "
    "``record_party_member`` when it is genuinely a new person. Never invent a "
    "birthday you were not given. Use ``record_dossier_inference`` only for "
