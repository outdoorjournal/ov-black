#!/usr/bin/env bash
# scripts/verify-s07.sh — single-command slice verification for S07.
#
# Mirrors scripts/verify-s06.sh: one run_check per acceptance bullet, each
...
#
# Hard-fails (not skips) if pnpm is missing from PATH — S07's acceptance
...
#
# Output is structured and prefixed with [verify-s07] so the log greps clean.
...
SUITE_FILE="s07-slice-acceptance"
...
# apps/web/tests/s07-slice-acceptance.test.tsx. We run them one at a time so
...
run_check "agent emits three cards during a test conversation" \
          "agent emits three cards during a test conversation"
run_check "a Card renders full detail from an ExperienceItem fixture" \
          "a Card renders full detail from an ExperienceItem fixture"
run_check "pin / keep / discard dispatches the right status transitions" \
          "pin / keep / discard dispatches the right status transitions"
run_check "reload hydration renders approved + proposed + discarded from initialCards" \
          "reload hydration renders approved"
run_check "craft-feel invariants in the mood-board subtree" \
          "craft-feel invariants in the mood-board subtree"
run_check "every card carries data-source and data-source-id matching the OV fixture" \
          "every card carries data-source"
...
log "All S07 acceptance bullets passed."
exit 0
