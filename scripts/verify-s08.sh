#!/usr/bin/env bash
# scripts/verify-s08.sh — single-command slice verification for S08.
#
# Mirrors scripts/verify-s07.sh: one run_check per acceptance bullet, each
...
#
# Hard-fails (not skips) if pnpm is missing from PATH — S08's acceptance
...
#   up jsdom + the RTL render path for the DraftItineraryEditor).
#
# Exit codes:
#   0   all acceptance bullets passed
#   1   any check failed or pnpm is not on PATH
#
# Output is structured and prefixed with [verify-s08] so the log greps clean.
...
SUITE_FILE="s08-slice-acceptance"
...
# apps/web/tests/s08-slice-acceptance.test.tsx. We run them one at a time so
...
run_check "draft editor renders assembled nodes from getItinerary fixture" \
          "draft editor renders assembled nodes from getItinerary fixture"
run_check "Edit button acquires lock and enables inline editors" \
          "Edit button acquires lock and enables inline editors"
run_check "Approve transitions status optimistically and disables the button on approved" \
          "Approve transitions status optimistically"
run_check "craft-feel invariants under data-testid='draft-itinerary-editor'" \
          "craft-feel invariants under data-testid"
run_check "Release dispatches releaseItineraryLock and clears lock indicator optimistically" \
          "Release dispatches releaseItineraryLock"
run_check "Non-advisor GET /itinerary/{id} on draft returns 403 mapped to detail='forbidden'" \
...
log "All S08 acceptance bullets passed."
exit 0
