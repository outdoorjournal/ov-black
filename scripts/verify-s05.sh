#!/usr/bin/env bash
# scripts/verify-s05.sh — single-command slice verification for S05.
#
# Mirrors scripts/verify-s04.sh: one run_check per acceptance bullet, each
...
#
# Hard-fails (not skips) if pnpm is missing from PATH — S05's acceptance
...
#
# Output is structured and prefixed with [verify-s05] so the log greps clean.
...
SUITE_FILE="s05-slice-acceptance"
...
# apps/web/tests/s05-slice-acceptance.test.tsx. We run them one at a time so
...
run_check "parseFrames splits one complete frame and carries the rest" \
          "parseFrames splits one complete frame"
run_check "streamed deltas land as concatenated text in the ChatShell DOM" \
          "streamed deltas land as concatenated text"
run_check "error frame renders D015 crafted-fallback copy and re-enables composer" \
          "error frame renders D015 crafted-fallback copy"
run_check "no progressbar / aria-busy / spinner / skeleton; assistant uses Cormorant" \
          "assistant uses Cormorant"
run_check "first delta text appears in the DOM within the 2000 ms budget" \
          "first delta text appears in the DOM within the 2000 ms budget"
run_check "no emoji in the rendered conversation DOM after an innocuous turn" \
          "no emoji in the rendered conversation DOM"
...
log "All S05 acceptance bullets passed."
exit 0
